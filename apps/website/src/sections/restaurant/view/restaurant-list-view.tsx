...
import { mockRestaurants, PUBLISH_OPTIONS } from '@src/_mock/restaurant';
...
export default function RestaurantDetailsView({ slug }: Props) {
  ...
  useEffect(() => {
    if (restaurant) {
      setPublish(restaurant?.isActive ? 'ACTIVE' : 'INACTIVE');
    }
  }, [restaurant]);
  ...
  const renderRestaurant = restaurant && (
    <>
      <RestaurantDetailsToolbar
        ...
        editLink={paths.restaurant.edit(`${restaurant?.id}`)}
